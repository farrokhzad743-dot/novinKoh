...
function hydrateImage(img, imageUrl) {
  if (!img || !imageUrl) { setImageFallback(img); return false; }
  img.dataset.pageUrl = imageUrl;
  img.onload = () => {
    img.classList.remove('image-failed');
    img.parentElement?.classList.remove('image-failed');
  };
  img.onerror = () => setImageFallback(img);
  img.src = imageUrl;
  return true;
}
